...
import { deployDatabase } from "./deploy-database.ts";
...
async function run() {
  ...
  switch (command) {
    ...
    case "deploy": {
      ...
      await deployDatabase(path.resolve(values.databaseDirectory));
      break;
    }
    case "notify": {
      const { values } = parseRequiredArgs(
    ...
    default:
      console.error(
        `Unknown command "${command ?? ""}". ` +
          "Usage: node src/update-database/index.ts <build|deploy|notify> [options...]",
      );
      process.exit(1);
  }
}

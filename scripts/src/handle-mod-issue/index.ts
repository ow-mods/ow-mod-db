...
const { values } = parseRequiredArgs(
  {
    ...
    required: ["modsFile", "issueTemplateFile", "issueNumber"],
  },
);

const { modsFile, issueTemplateFile, issueNumber } = values;
...
async function findReportComment(
  ...
  repo: string,
  issueNumber: number,
  issueCommentBotUserName: string,
) {
  ...
  const reportComment = comments.find(
    (comment) =>
      comment.user?.login === issueCommentBotUserName &&
...
async function updateOrCreateReportComment(
  ...
  const existingId =
    commentId ??
    (await findReportComment(
      owner,
      repo,
      issueNumber,
      issueCommentBotUserName,
    ));
  ...
  if (existingId) {
    try {
      ...
      console.warn("Failed to update report comment: ", error);

      const foundId = await findReportComment(
        owner,
        repo,
        issueNumber,
        issueCommentBotUserName,
...
async function run() {
  ...
  const postComment = (body: string, commentId?: number) =>
    updateOrCreateReportComment(
      DATABASE_REPOSITORY.owner,
